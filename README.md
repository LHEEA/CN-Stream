...
### Input file
The input file is assumed to be named `CN_Stream_input.dict`. An example is provided in the `input` folder and details are provided hereafter. The `Options_solver` parameters are useful for an advanced user, in order to obtain solutions with a controlled accuracy and/or to look for waves close to the wave breaking limit.
...
- `WriteOutput`: this subroutine creates the file resultsOutput.txt containing at a given location and time all spatial quantities computed by CN-Stream (free surface elevation, velocities, pressures, derivaitves, etc.).
